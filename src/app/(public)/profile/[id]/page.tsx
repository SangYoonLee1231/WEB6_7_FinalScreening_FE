import Avatar from "@/components/common/Avatar";
import { BoxButton } from "@/components/common/button/BoxButton";
import HorizontalCardContainer from "@/components/common/container/HorizontalCardContainer";
import IntroduceBubble from "@/components/profile/IntroduceBubble";
import TierSet from "@/components/profile/TierSet";
import WinRate from "@/components/profile/WinRate";
import ReviewCard from "@/components/review/ReviewCard";
import ReviewPercent from "@/components/review/ReviewPercent";

export default function ProfilePage() {
  const rations = { GOOD: 3, NORMAL: 6, BAD: 1 };
  return (
    <section className="flex h-full w-full">
      <div className="flex h-full w-full flex-col gap-9">
        {/* 프로필 정보 */}
        <div className="mt-20 flex flex-col gap-5">
          <div className="flex flex-row gap-1 text-5xl">
            <p className="text-accent">커뮤니티닉네임</p>
            <p className="text-content-main">님의 프로필</p>
          </div>
          <div className="flex flex-row items-center gap-8">
            <Avatar size="xxl" src="" type="profile" />
            <div className="flex flex-col gap-4">
              <div className="flex flex-row items-center justify-between">
                <p className="text-content-primary text-[28px] font-semibold">
                  커뮤니티 닉네임
                </p>
                <BoxButton
                  size="sm"
                  tone="negative"
                  className="w-12 py-3"
                  text="차단"
                />
              </div>
              <IntroduceBubble
                size="lg"
                content="인간 시대의 끝이 도래했다"
                className="h-11.5"
              />
            </div>
          </div>
        </div>
        {/* 연동된 게임 정보 */}
        <div className="flex flex-col gap-8">
          <p className="text-content-main text-[32px] font-bold">
            연동된 게임 정보
          </p>
          {/* 게임 탭*/}
          {/* 게임 프로필 정보 및 전적 갱신 버튼 */}
          <div className="flex flex-row items-center gap-8">
            <div className="flex flex-row items-center gap-3">
              <Avatar type="champion" src="" />
              <div className="flex flex-row items-center gap-1">
                <p className="text-content-primary text-xl font-bold">
                  게임닉네임
                </p>
                <p className="text-content-secondary text-sm">#1234</p>
              </div>
            </div>
            <BoxButton
              tone="color"
              size="sm"
              className="w-20 py-3"
              text="전적 갱신"
            />
          </div>
          {/* 랭크 및 승률 정보 */}
          <div className="grid grid-cols-[1fr_1fr_1fr] gap-5">
            <HorizontalCardContainer className="flex flex-col items-center justify-center gap-8 border-none px-12 py-6">
              <p className="text-semibold text-xl">개인/2인 랭크 게임</p>
              <div className="flex flex-col">
                <TierSet
                  tier="DIAMOND"
                  rank="I"
                  className="w-27"
                  type="default"
                />
                <p className="text-content-secondary text-sm">RANK</p>
              </div>
            </HorizontalCardContainer>
            <HorizontalCardContainer className="flex flex-col items-center justify-center gap-8 border-none px-12 py-6">
              <p className="text-semibold text-xl">자유 랭크 게임</p>
              <div className="flex flex-col">
                <TierSet
                  tier="DIAMOND"
                  rank="I"
                  className="w-27"
                  type="default"
                />
                <p className="text-content-secondary text-sm">RANK</p>
              </div>
            </HorizontalCardContainer>
            <HorizontalCardContainer className="flex flex-col items-center justify-center gap-5.5 border-none px-12 py-6">
              <p className="text-semibold text-xl">승률</p>
              <WinRate type="donut" className="w-25" />
            </HorizontalCardContainer>
          </div>
          {/* 최근선호 챔피언 */}
          {/* 최근 게임 내역 */}
        </div>

        {/* 리뷰 내역 */}
        <div className="flex flex-col gap-4">
          <p className="text-content-main text-[32px] font-bold">리뷰 내역</p>
          <div className="flex flex-col gap-12.5">
            {/* 리뷰 분포 */}
            <ReviewPercent type="default" ratios={rations} />
            {/* 리뷰 상세 내역 */}
            <div className="flex flex-col gap-7.5">
              <p className="text-content-secondary text-base text-center">총 12개의 리뷰</p>
              <div className="flex flex-col gap-2">
                <ReviewCard
                  mode="received"
                  gameName="lol"
                  communityName="커뮤니티 닉네임"
                  content="리뷰내용"
                  emotion="good"
                  createdAt="2025-12-12T00:12:00.000Z"
                  profileImageURL=""
                />
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
